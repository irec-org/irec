...
from irec.utils.PersistentDataManager import PersistentDataManager
                try:
                    ...
                    datasets_metrics_values[dataset_preprocessor['name']][
                        metric_class_name][agent.name][json.dumps(
                            parameters)] = metric_values[-1]
                except:
                    print(
                        f"Could not load metric {dataset_preprocessor['name']} {itr_class.__name__}"
                    )
                    pass
...
for k1, v1 in datasets_metrics_values.items():
    for k2, v2 in v1.items():
        for k3, v3 in v2.items():
            ...
            if args.d:
                settings['agents_preprocessor_parameters'][k1][
                    k3] = json.loads(keys[0])
            if args.t:
                print(f"{k3}:")
                # print('\tparameters:')
