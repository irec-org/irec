    ...
    def evaluate(self, model, train_dataset, test_dataset):
        with threadpool_limits(limits=1, user_api='blas'):
            ...
            test_consumption_matrix = scipy.sparse.csr_matrix(
                ...
                shape=(test_dataset.num_total_users,
                       test_dataset.num_total_items))
            ...
            for i in range(num_trials):
                ...
                for item in best_items:
                    ...
                    model.observe(None, (uid, item),
                                  test_consumption_matrix[uid, item], info)
...
class InteractionSample(EvaluationPolicy):
    ...
    def evaluate(self, model, train_dataset, test_dataset):
        ...
        num_total_items = test_dataset.num_total_items
        test_consumption_matrix = scipy.sparse.csr_matrix(
            (test_dataset.data[:, 2], (test_dataset.data[:, 0].astype(int),
                                       test_dataset.data[:, 1].astype(int))),
        ...
        users_sampled = np.random.choice(uids,
                                         num_users_to_sample,
                                         replace=False)
        ...
        for i in range(num_trials):
            ...
            best_items = items_not_recommended[np.argpartition(
                items_score, -self.interaction_size)[-self.interaction_size:]]
            users_items_recommended[uid].extend(best_items)
        ...
        for uid, items in users_items_recommended.items():
            ...
            plt.scatter(items_entropy,
                        items_popularity,
                        s=100,
                        color='#d1d1d1')

            plt.scatter(items_entropy[items],
                        items_popularity[items],
                        s=100,
                        color=colors)
...
class OneInteraction(EvaluationPolicy):
    ...
    def evaluate(self, model, train_dataset, test_dataset):
        with threadpool_limits(limits=1, user_api='blas'):
            ...
            membership_table = []
            top_k_nonp = 100
            top_k_nonp_items_popularity = np.argsort(
                items_popularity)[::-1][:top_k_nonp]
            top_k_nonp_items_entropy = np.argsort(
                items_entropy)[::-1][:top_k_nonp]
            ...
            for i in range(num_trials):
                ...
                if users_num_interactions[uid] == 1:
                    ...
                    items_value['Entropy'][uid] = np.mean(
                        items_entropy[best_items])
                    membership['Popularity'][uid] = len(
                        set(best_items).intersection(
                            set(top_k_nonp_items_popularity)))
                    membership['Entropy'][uid] = len(
                        set(best_items).intersection(
                            set(top_k_nonp_items_entropy)))
                ...
                if len(no_items_recommended_users) == 0:
                    ...
                    # ))
                    print('Items_Values: {} {} {} {}'.format(
                        train_dataset.num_total_users,
                        train_dataset.num_total_items, model.name, ' '.join([
                            '{} {}'.format(corr_name,
                                           np.mean(list(corr_values.values())))
                            for corr_name, corr_values in items_value.items()
                        ])))
                    print('Membership: {} {} {} {}'.format(
                        train_dataset.num_total_users,
                        train_dataset.num_total_items, model.name, ' '.join([
                            '{} {}'.format(corr_name,
                                           np.mean(list(corr_values.values())))
