...
class DefaultDataset(DataProcessor):
    ...
    def filter_users(self, df_dataset, filters):
        ...
        def min_consumption_users(df_dataset, min_consumption):
        ...
        def number_users(df_dataset, num_users):
            try: selected_users = random.sample(list(df_dataset[0].unique()), num_users)
            except: return df_dataset
            return df_dataset[df_dataset[0].isin(selected_users)]

        for filter_user in filters:
            if filter_user == "min_consumption":
                df_dataset = min_consumption_users(df_dataset, filters[filter_user])
            elif filter_user == "num_users":
                df_dataset = number_users(df_dataset, filters[filter_user])

        return df_dataset

    def filter_items(self, df_dataset, filters):

        def min_ratings_items(df_dataset, min_ratings):
        ...
        def number_items(df_dataset, num_items):
            try: selected_items = random.sample(list(df_dataset[1].unique()), num_items)
            except: return df_dataset
            return df_dataset[df_dataset[1].isin(selected_items)]

        for filter_item in filters:
            if filter_item == "min_ratings":
                df_dataset = min_ratings_items(df_dataset, filters[filter_item])
            elif filter_item == "num_items":
                df_dataset = number_items(df_dataset, filters[filter_item])
