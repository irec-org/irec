...
from irec.agents.value_functions.experimental.log_pop_ent import LogPopEnt
from irec.agents.value_functions.experimental.best_rated import BestRated
...
class PercentageInteraction(EvaluationPolicy):
    ...
    def rec_npers(parameters):
        ...
        num_items = 0
        rec_items = []
        top_k_items = random.sample(list(items_not_recommended), len(list(items_not_recommended))) if method == "Random" else self.nonp_methods[method]
        ...
        items_not_recommended = (np.nonzero(not_recommended)[0])
        return [uid, int((itr/self.interaction_size_npers)+0.5), rec_items, items_not_recommended, not_recommended]
    ...
    def rec_mab(self, result, model):
        ...
        for itr in range(self.num_interactions):
            ...
            for item in best_items:
                ...
                if self.test_consumption_matrix[uid, item] >= self.threshold: 
                    model.observe(None, (uid, item), self.test_consumption_matrix[uid, item], info)
        ...
        with ProcessPoolExecutor(max_workers=16) as executor:
            for method in self.nonp_methods:
                ...
                for exchange_point in self.exchange_points:
                    ...
                    self_id = id(self)
                    parameters = [[self_id], [method], [exchange_point], available_users]
                    parameters = list(itertools.product(*parameters)) 

                    pbar = tqdm(executor.map(PercentageInteraction.rec_npers, parameters),total=len(parameters))
                    pbar.set_description(f"{current_itr}/{itr} Stage 1 - {method} {exchange_point}")

                    results = [i for i in pbar]
                    update = [rec_items for result in results for rec_items in result[2] if rec_items[2] >= 4]

                    updated_train_dataset = Dataset(data=np.concatenate((train_dataset.data, update)))
                    ...
                    for [uid,item,rating,timestamp] in update:
                        current_model.observe(None, (uid, item), rating, {"vf_info": None, "asp_info": None})                

                    pbar = tqdm(results, position=0, leave=True)
                    pbar.set_description(f"{current_itr}/{itr} Stage 2 - MAB Problem")
                    
                    history_items_recommended[method][exchange_point] = [self.rec_mab(result, current_model) for result in pbar]
                    current_itr += 1
