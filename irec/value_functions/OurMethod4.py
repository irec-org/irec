...
class OurMethod4(MFValueFunction):
    ...
    def reset(self, observation):
        ...
        self.items_bias = value_functions.LogPopEnt.get_items_logpopent(
            items_popularity, items_entropy, self.lambda_)
        ...
        print(
            f"WSCB {self.get_id(with_class_name=False)} items score correlation with popularity:",
            scipy.stats.pearsonr(items_score, items_popularity),
            self.train_dataset.num_total_users,
            self.train_dataset.num_total_items)
        print(
            f"WSCB {self.get_id(with_class_name=False)} items score correlation with entropy:",
            scipy.stats.pearsonr(items_score, items_entropy),
            self.train_dataset.num_total_users,
            self.train_dataset.num_total_items)
