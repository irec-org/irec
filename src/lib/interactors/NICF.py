...
class FA(basic_model):
    def _create_placeholders(self):
        ...
        self.p_rec = [tf.compat.v1.placeholder(tf.int32,(None,None,), name="p"+str(i)+"_rec") for i in range(6)]
        self.pt = [tf.compat.v1.placeholder(tf.int32,(None,2),"p"+str(i)+"t") for i in range(6)]
    ...
    def _create_inference(self):
        ...
        i_p_mask = [tf.expand_dims(tf.compat.v1.to_float(tf.not_equal(item, 0)), -1) for item in self.p_rec]
        ...
        for iii,item in enumerate(self.p_seq):
            for i in range(self.args.num_blocks):
                with tf.compat.v1.variable_scope("rate_"+str(iii)+"_num_blocks_"+str(i)):
        ...
        hidden = tf.compat.v1.layers.dense(context,self.args.latent_factor,activation=tf.nn.relu)
    ...
    def _create_optimizer(self):
        ...
        self.optimizer = tf.compat.v1.train.AdamOptimizer(self.args.learning_rate).minimize(self.loss)
...
class NICF(ExperimentalInteractor):
    def __init__(self,time_step, latent_factor, learning_rate, training_epoch, rnn_layer, inner_epoch, batch, gamma, clip_param, restore_model, num_blocks, num_heads, dropout_rate, *args, **kwargs):
        ...
        self.parameters.extend(['time_step', 'latent_factor', 'learning_rate', 'training_epoch', 'rnn_layer', 'inner_epoch', 'batch', 'gamma', 'clip_param', 'restore_model', 'num_blocks', 'num_heads', 'dropout_rate'])
    ...
    def train_epoch(self,epoch):
        ...
        for uuid in selected_users:
            ...
            while not done:
                ...
                self.memory.append([s_pre,action,rwd,done,copy.deepcopy(state_next)])
                actions[action] = 1
                state = state_next
