...
class kNNBandit(ExperimentalInteractor):
    ...
    def predict(self, uid, candidate_items, num_req_items):
        ...
        for i, v1, v2 in zip(list(range(self.num_total_users)),
                             self.users_alphas[uid],
                             self.users_rating_sum - self.users_alphas[uid]):
            if v1 > 0 and v2 > 0:
        ...
        return self.consumption_matrix[top_user].A.flatten()[candidate_items], {
            'top_user': top_user
        }
