...
class PersistentDataManager(DirectoryDependent):
    def __init__(self,
                 directory='state_save',
                 extension_name='.pickle',
                 *args,
                 **kwargs):
    ...
    def get_fp(self, path):
        fp = os.path.join(self.DIRS[self.directory],
                          path + self.extension_name)
    ...
    def json_entry_save_format(uid, items):
        return json.dumps({
            'uid': int(uid),
            'predicted': list(map(int, items))
        }) + '\n'

    def save_results(self, path, results, data_type='pickle'):
        if 'pickle' in data_type:
            with open(f'{os.path.join(self.DIRS["result"],path)}.pickle',
                      "wb") as f:
                pickle.dump(self.results, f)
        if 'txt' in data_type:
            with open(f'{os.path.join(self.DIRS["result"],path)}.txt',
                      "w") as f:
