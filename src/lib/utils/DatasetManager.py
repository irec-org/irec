...
import sys
sys.path.append(
    dirname(realpath(__file__)) + sep + pardir + sep + pardir + sep + "lib")
...
class DatasetManager(Parameterizable):
    ...
    def initialize_engines(self, dataset_preprocessor):
        ...
        with open("settings" + sep +
                  "processors_parameters.yaml") as processors_parameters_f:
            # loader = yaml.SafeLoader
            ...
            for data_processor in dataset_preprocessor['preprocessor']:
                ...
                pipeline.steps.append(
                    eval('dataset.' + data_processor_name)(**parameters))
                # else:
        ...
        self.dataset_preprocessor = dataset.DatasetPreprocessor(
            dataset_preprocessor['name'], dataset_descriptor, preprocessor)

    def run_preprocessor(self):
        self.dataset_preprocessed = self.dataset_preprocessor.preprocessor.process(
            self.dataset_preprocessor.dataset_descriptor)
