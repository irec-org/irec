        ...
        for i, values in enumerate(combinations):
            ...
            if self.n_splits:
                for train_index, test_index in kf.split(X_data,y=X_row):
                    X_train = scipy.sparse.csr_matrix((X_data[train_index],(X_row[train_index],X_col[train_index])),shape=X.shape)
                    X_test = scipy.sparse.csr_matrix((X_data[test_index],(X_row[test_index],X_col[test_index])),shape=X.shape)
