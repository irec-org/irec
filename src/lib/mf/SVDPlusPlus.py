...
@jit(nopython=True)
def _svdplusplus(data, indptr, indices, n_u, num_users, num_items, num_lat, learn_rate, delta, delta_bias, bias_learn_rate, iterations, stop_criteria,init_mean,init_std):
    # print(n_u)
    num_r = len(data)
    r_mean = np.mean(data)
    r_std = np.std(data)
