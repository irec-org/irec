...
\hline
\rowcolor{StrongGray}
Dataset & %s \\""" % (utils.generate_table_spec(nums_interactions_to_show,
                                                len(datasets_preprocessors)),
                      utils.generate_datasets_line(
                          nums_interactions_to_show,
...
datasets_interactors_items_recommended = defaultdict(lambda: defaultdict(list))

for dataset_preprocessor in datasets_preprocessors:
    dm.initialize_engines(dataset_preprocessor)
    for itr_class in interactors_classes:
        # itr = ir.create_interactor(itr_class)
        itr = utils.create_interactor(itr_class, dataset_preprocessor['name'],
                                      settings)
        ...
        datasets_interactors_items_recommended[dataset_preprocessor['name']][
            itr_class.__name__] = users_items_recommended
...
for dataset_preprocessor in datasets_preprocessors:
    ...
    if args.type == 'users':
        ...
        ground_truth_consumption_matrix = scipy.sparse.csr_matrix(
            (ground_truth_dataset.data[:, 2],
             (ground_truth_dataset.data[:, 0], ground_truth_dataset.data[:,
                                                                         1])),
    ...
    for nits in nums_interactions_to_show:
        ...
        array[array == 0] = 314
        dfs[nits] = pd.DataFrame(
            array,
            index=[
                interactors_classes_names_to_names[i.__name__]
                for i in interactors_classes
            ],
            columns=[
                interactors_classes_names_to_names[i.__name__]
                for i in interactors_classes
    ...
    for ii, itr_class_1 in enumerate(interactors_classes):
        itr_1 = utils.create_interactor(itr_class_1,
                                        dataset_preprocessor['name'], settings)
        for jj, itr_class_2 in enumerate(interactors_classes):
            if ii > jj:
                itr_2 = utils.create_interactor(itr_class_2,
                ...
                if args.type == 'Users Jaccard Similarity':
                    ...
                    for i in nums_interactions_to_show:
                        x = set()
                        for uid, items in itr_1_recs.items():
                            if np.sum(ground_truth_consumption_matrix[
                                    uid, items[:i]] >= 4) > 0:
                                x.add(uid)
                        y = set()
                        for uid, items in itr_2_recs.items():
                    ...
                    for i in nums_interactions_to_show:
                        ...
                        for uid, items in itr_1_recs.items():
                            sample_rec_items = list(
                                set(items[:i]).intersection(
                                    set(itr_2_recs[uid][:i])))
                ...
                for iii, nits in enumerate(nums_interactions_to_show):
                    dfs[nits][interactors_classes_names_to_names[
                        itr_class_1.__name__]][
                            interactors_classes_names_to_names[
                                itr_class_2.__name__]] = vals[iii]
                    dfs[nits][interactors_classes_names_to_names[
                        itr_class_2.__name__]][
                            interactors_classes_names_to_names[
                                itr_class_1.__name__]] = vals[iii]

                datasets_metrics_values[dataset_preprocessor['name']][
                    args.type][name].extend(vals)
    ...
    for iii, nits in enumerate(nums_interactions_to_show):
        ...
        file_name = os.path.join(
            ...
            f'{dataset_preprocessor["name"]}_{args.type}',
            f'cm_{evaluation_policy.num_interactions}_{evaluation_policy.interaction_size}',
            f'cm_{nits}.png')
...
file_name_suffix = os.path.join(
    f'table_similarity_{args.type}_{evaluation_policy.num_interactions}_{evaluation_policy.interaction_size}_{tmp}.tex'
)
