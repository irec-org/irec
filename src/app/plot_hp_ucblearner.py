...
for value in range(0, 51):
    ...
    for j in tqdm(range(len(KS))):
        k = KS[j]
        me = MetricsEvaluator(name=itr.get_id(),
                              k=k,
                              threshold=THRESHOLD,
...
fig.suptitle(
    f"Top-{INTERACTION_SIZE} recommendation with {INTERACTIONS} interactions")
for ax, metric_name in zip(axs.flatten(), metrics_names):
    x, y = np.array(list(metric_values[metric_name].keys())), np.array(
        list(metric_values[metric_name].values()))
    ax.plot(x, y, color='k')
    argmax = np.argmax(y)
    colors = (['k'] * len(x))
    colors[argmax] = 'r'
    ...
    ax.set_title("Best {} at {}".format(
        MetricsEvaluator.METRICS_PRETTY[metric_name], argmax))
...
plt.savefig(f'img/hp_ucblearner_{INTERACTIONS}_{INTERACTION_SIZE}.png',
            bbox_inches='tight')
plt.savefig(f'img/hp_ucblearner_{INTERACTIONS}_{INTERACTION_SIZE}.eps',
            bbox_inches='tight')
