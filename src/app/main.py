...
@click.option(
    ...
    default='ml_100k',
    help=
    f'Dataset to use [{", ".join(list(DatasetFormatter.BASES_DIRS.keys()))}]')
@click.option(
    '-s',
    default='users_train_test',
    help=
    f'Selection model [{", ".join(list(DatasetFormatter.SELECTION_MODEL.keys()))}]'
)
@click.pass_context
def cli1(ctx, d, s):
