...
metrics_names = [
    'Cumulative Precision', 'Cumulative Recall', 'Cumulative EPC',
    'Cumulative Users Coverage', 'Cumulative ILD', '1-(Gini-Index)'
]
metrics_weights = {
    'Hits': 0.3,
    'Recall': 0.3,
...
for dataset_preprocessor in datasets_preprocessors:
    ...
    items_popularity = lib.value_functions.MostPopular.get_items_popularity(
        ground_truth_consumption_matrix, normalize=False)
    print(dataset_preprocessor['name'])
    print("\tpopularity correlation with entropy = %s" %
          str(scipy.stats.pearsonr(items_popularity, items_entropy)))
    items_logpopent = lib.value_functions.LogPopEnt.get_items_logpopent(
    ...
    for k in np.linspace(0, 1, 11):
        print('----', k, '----')
        items_logpopent = lib.value_functions.LogPopEnt.get_items_logpopent(
            items_popularity, items_entropy, k=k)
        pe = scipy.stats.pearsonr(items_logpopent, items_entropy)
        pp = scipy.stats.pearsonr(items_logpopent, items_popularity)
        print("\tent^%.1f*log(pop)^%.1f correlation with entropy = %s" %
              (k, 1 - k, str(pe)))
