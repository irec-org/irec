...
def process(history_rate, dataset_preprocessor, dataset, consumption_matrix,
    ...
    pdm = PersistentDataManager(directory='results')
        ...
        if not pdm.file_exists(file_name_s) or args.f1:
            ...
            pdm.save(file_name_s, history_items_recommended)
        ...
        if not pdm.file_exists(file_name) or args.f2:
            ...
            for (user, item) in history_items_recommended:
                if consumption_matrix[user, item] > 0:
                    new_data.append((user, item, consumption_matrix[user,
                                                                    item]))
            new_train_data = np.array(new_data)
            train_data = dm.dataset_preprocessed[0].data
            new_train_data = np.vstack((train_data[:, 0:3], new_data))
            ...
            history_items_recommended = last_evaluation_policy.evaluate(
                itr, train_dataset, test_dataset)
            pdm.save(file_name, history_items_recommended)
        else:
            print("File already exists")
    except:
        print("Error Ocurred !!!!!!!!")
