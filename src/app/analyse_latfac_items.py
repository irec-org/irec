...
rep_logpopent_corr = np.corrcoef(items_representativeness, items_logpopent)[0,
                                                                            1]
...
for i in range(num_items):
    ...
    table.append(
        ("{}({:.2f})\t{:.2f}({})\t{}({})\t{:.2f}({})\t{:.2f}({})".format(
            item, np.mean(dsf.consumption_matrix[:, item].data), item_rep,
            item_rank_rep, item_pop, item_rank_pop, item_ent, item_rank_ent,
            item_logpopent, item_rank_logpopent)).split('\t'))

print(
    tabulate(table,
             headers=[
                 'Item ID(Rat. mean)', 'Representativeness(Rank)',
                 'Popularity(Rank)[c:%.2f]' % (rep_pop_corr),
                 'Entropy(Rank)[c:%.2f]' % (rep_ent_corr),
                 'LogPopEnt(Rank)[c:%.2f]' % (rep_logpopent_corr)
             ]))
