...
def get_groups_and_methods_metrics_from_sample(itr_1_recs, itr_2_recs,
                                               ground_truth_consumption_matrix,
                                               type_):
    data = dict()
    data['hits'] = 0
    ...
    for uid, rec_items in itr_1_recs.items():
        if type_ == 'inter':
            sample_rec_items = list(
                set(rec_items).intersection(set(itr_2_recs[uid])))
        elif type_ == 'x-y':
        ...
        data['hits'] += np.sum(
            ground_truth_consumption_matrix[uid, sample_rec_items] >= 4)
...
for dataset_preprocessor in datasets_preprocessors:
    ...
    for itr_class in interactors_classes:
        ...
        datasets_interactors_items_recommended[dataset_preprocessor['name']][
            itr_class.__name__] = uir
...
for dataset_preprocessor in datasets_preprocessors:
    ...
    for ii, itr_class_1 in enumerate(interactors_classes):
        ...
        itr_1 = itr_class(**settings['interactors_preprocessor_parameters'][
            dataset_preprocessor['name']][itr_class_1.__name__]['parameters'])
        for jj, itr_class_2 in enumerate(interactors_classes):
            if ii > jj:
                ...
                itr_2 = itr_class(
                    **settings['interactors_preprocessor_parameters'][
                        dataset_preprocessor['name']][
                            itr_class_2.__name__]['parameters'])
                name = interactors_classes_names_to_names[
                    itr_class_1.
                    __name__] + r' $\times $ ' + interactors_classes_names_to_names[
                        itr_class_2.__name__]
                itr_1_recs = datasets_interactors_items_recommended[
                    dataset_preprocessor['name']][itr_class_1.__name__]
                itr_2_recs = datasets_interactors_items_recommended[
                    dataset_preprocessor['name']][itr_class_2.__name__]
                vals = []
                x = set()
                y = set()
                for uid1, items1 in itr_1_recs.items():
                    items2 = itr_2_recs[uid1]
                    v = len(set(items1).intersection(
                        set(items2))) / len(items1)
                ...
                file_name = os.path.join(
                    DirectoryDependent().DIRS['img'], 'similarity',
                    f'{dataset_preprocessor["name"]}_{interactors_classes_names_to_names[itr_class_1.__name__]}_{interactors_classes_names_to_names[itr_class_2.__name__]}.png'
                )
                ...
                mni_g1, hits_g1 = get_groups_and_methods_metrics_from_sample(
                    itr_1_recs,
                    itr_2_recs,
                    ground_truth_consumption_matrix,
                    type_='inter')
                mni_g2, hits_g2 = get_groups_and_methods_metrics_from_sample(
                    itr_1_recs,
                    itr_2_recs,
                    ground_truth_consumption_matrix,
                    type_='x-y')
                mni_g3, hits_g3 = get_groups_and_methods_metrics_from_sample(
                    itr_2_recs,
                    itr_1_recs,
                    ground_truth_consumption_matrix,
                    type_='x-y')
                # mni_g4, hits_g4 = get_groups_and_methods_metrics_from_sample(g4,itr_2_recs,ground_truth_consumption_matrix)
                mni_itr_1, hits_itr_1 = get_groups_and_methods_metrics_from_sample(
                    itr_1_recs,
                    None,
                    ground_truth_consumption_matrix,
                    type_=None)
                mni_itr_2, hits_itr_2 = get_groups_and_methods_metrics_from_sample(
                    itr_2_recs,
                    None,
                    ground_truth_consumption_matrix,
                    type_=None)
                ...
                results.append([
                    '', 'G1', 'G2', 'G3',
                    interactors_classes_names_to_names[itr_class_1.__name__],
                    interactors_classes_names_to_names[itr_class_2.__name__]
                ])
                results.append([
                    dataset_preprocessor['name'], hits_g1, hits_g2, hits_g3,
                    hits_itr_1, hits_itr_2
                ])
                # results.append(['Num. Items',len(g1),len(g2),len(g3),len(all_items),len(all_items)])
                results.append([
                    'Num. Mean Items', mni_g1, mni_g2, mni_g3, mni_itr_1,
                    mni_itr_2
                ])
