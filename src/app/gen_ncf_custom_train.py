...
def process(history_rate, dataset_preprocessor, dataset, consumption_matrix,
            dm, interactor_class, timestamp_matrix):
    try:
        ...
        train_idx, test_idx = next(
            gss.split(train_data, groups=train_data[:, 0].astype(int)))
        ...
        train_data = dm.dataset_preprocessed[0].data
        train_idx, test_idx = next(
            gss.split(train_data, groups=train_data[:, 1].astype(int)))
        data = train_data[train_idx]
        ...
        parameters = interactors_preprocessor_parameters[
        ...
        with open('support.csv'.format(dataset_preprocessor['name']),
                  'w') as f:
            for i in range(len(data)):
                uid = int(data[i, 0])
                iid = int(data[i, 1])
        ...
        for (user, item) in history_items_recommended:
            if consumption_matrix[user, item] > 0:
                new_data.append((user, item, consumption_matrix[user, item],
                                 timestamp_matrix[user, item]))
        new_train_data = np.array(new_data)
        train_data = dm.dataset_preprocessed[0].data
        new_train_data = np.vstack((train_data[:, 0:4], new_data))
        test_data = dm.dataset_preprocessed[1].data
        new_test_data = test_data[~(
            np.isin(test_data[:, 0], set(new_train_data[:, 0]))
            & np.isin(test_data[:, 1], set(new_train_data[:, 1])))]
        ...
        with open('{}.train.rating'.format(dataset_preprocessor['name']),
                  'w') as f:
            for i in range(len(data)):
                uid = int(data[i, 0])
                iid = int(data[i, 1])
        ...
        print("Test writing")
        with open('{}.test.rating'.format(dataset_preprocessor['name']),
                  'w') as f:
            for i in range(len(data)):
                uid = int(data[i, 0])
                iid = int(data[i, 1])
        ...
        with open('{}.test.negative'.format(dataset_preprocessor['name']),
                  'w') as f:
            for i in range(len(data)):
                uid = int(data[i, 0])
                iid = int(data[i, 1])
                # rating = data[i,2]
                # timestamp = int(data[i,3])
                user_negative_items = list(
                    map(str, list(all_items - users_consumed_items[uid])))
                sampled_negative_items = random.sample(user_negative_items, 99)
                f.write('({},{})\t{}\n'.format(
                    uid, iid, '\t'.join(sampled_negative_items)))
    except:
        print("Error Ocurred !!!!!!!!")
        traceback.print_exc()
        raise SystemError
...
with concurrent.futures.ProcessPoolExecutor(
    ...
    for dataset_preprocessor in datasets_preprocessors:
        ...
        timestamp_matrix = scipy.sparse.csr_matrix(
            (dataset.data[:, 3],
             (dataset.data[:, 0].astype(int), dataset.data[:, 1].astype(int))),
            shape=(dataset.num_total_users, dataset.num_total_items))
        for history_rate in history_rates_to_train:
            ...
            for interactor_class in interactors_classes:
                ...
                f = executor.submit(process, history_rate,
                                    dataset_preprocessor, dataset,
                                    consumption_matrix, dm, interactor_class,
                                    timestamp_matrix)
                futures.add(f)
