...
class ItemKNN(Recommender):
    ...
    def train(self,train_matrix):
        ...
        for iid in range(train_matrix.shape[1]):
            ...
            self.items_similar_items[iid] = top_iids[top_iids != iid][:self.num_neighbors]
    ...
    def predict(self,test_matrix):
        ...
        with threadpool_limits(limits=1, user_api='blas'):
            args = [(self_id,int(uid),) for uid in test_uids]
    ...
    def predict_user_items(obj_id, uid):
        ...
        for i, candidate_iid in enumerate(candidate_iids):
            ...
            #     continue
            neighbors_similarities = self.sim_matrix[candidate_iid,neighboring_items]
            # neighbors_similarities[neighbors_similarities==0] = 0.001
            numerator = np.sum(neighbors_similarities * (user_consumption[neighboring_items] - self.items_ratings_mean[neighboring_items]))
            denominator = np.sum(neighbors_similarities)
            # try:
            candidate_items_score[i] = numerator/denominator + self.items_ratings_mean[candidate_iid]
            # except:
    ...
    def filter_parameters(self,parameters):
        return super().filter_parameters({k: v for k, v in parameters.items() if k not in ['num_neighbors']})
